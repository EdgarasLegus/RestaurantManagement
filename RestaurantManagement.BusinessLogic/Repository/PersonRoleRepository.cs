...
namespace RestaurantManagement.BusinessLogic.Repository
{
    public class PersonRoleRepository : IPersonRoleRepo
    {
        ...
        public async Task InsertInitialPersonRoles(List<PersonRole> tableColumns)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                if (!_context.PersonRole.Any())
                {
                    ...
                    {
                        var personRoleEntity = new PersonRole()
                        {
                            Id = item.Id,
                            RoleName = item.RoleName
                        };
                        await _context.PersonRole.AddAsync(personRoleEntity);
                    }
